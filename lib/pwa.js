...
export const getPwaConfig = ({ siteInfo, notionConfig } = {}) => {
  ...
  // Priority: dedicated PWA_ICON_192/512 → PWA_ICON → siteInfo.icon → built-in default
  // This preserves backward compatibility while allowing explicit sized-icon overrides
  const fallbackIcon = notionConfig.PWA_ICON || siteInfo.icon
  const hasCustomIcon192 = !!(notionConfig.PWA_ICON_192 || fallbackIcon)
  const hasCustomIcon512 = !!(notionConfig.PWA_ICON_512 || fallbackIcon)
  ...
  // Maskable: dedicated maskable → built-in maskable default (when no custom icon) → fall back to any-purpose icon
  const icon192Maskable = notionConfig.PWA_ICON_192_MASKABLE ||
    (hasCustomIcon192 ? icon192 : DEFAULT_ICON_192_MASKABLE)
  const icon512Maskable = notionConfig.PWA_ICON_512_MASKABLE ||
    (hasCustomIcon512 ? icon512 : DEFAULT_ICON_512_MASKABLE)
