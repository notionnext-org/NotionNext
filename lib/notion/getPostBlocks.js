  ...
  return await getOrSetDataWithCache(
    ...
    async (id, slice) => {
      ...
      if (pageBlock) {
        ...
        return await convertNotionBlocksToPost(id, pageBlock, slice)
      }
 ...
 * @returns
 */
async function convertNotionBlocksToPost(id, blockMap, slice) {
  ...
  for (let i = 0; i < blocksToProcess.length; i++) {
    ...
    if( b?.value?.type === 'video'&& b?.value?.properties?.source?.[0][0] ){
      const body=[{
            "url": b?.value?.properties?.source?.[0][0],
            "permissionRecord": {
                "table": "block",
                "id":  b?.value?.id
            }
        }]
      const result =  await notionAPI.getSignedFileUrls(body)
      // console.log(result)
      b.value.properties.source[0][0] = result.signedUrls
    }
